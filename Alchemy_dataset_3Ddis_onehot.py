...
class TencentAlchemyDataset(InMemoryDataset):
    ...
    def __init__(self, root, processed_dir='processed', mode='dev', transform=None, pre_transform=None, pre_filter=None):
        self.mode = mode
        assert mode in _urls
        # print(transform)
        super(TencentAlchemyDataset, self).__init__(root, processed_dir, transform, pre_transform, pre_filter)
        print(f"Process dir: {self.processed_dir}")
        print(f"Data path: {self.processed_paths[0]}")
    ...
    def raw_file_names(self):
        if self.mode=='dev':
            ...
            return [self.mode + '/sdf', self.mode + '/valid.csv']
        else:
            return [self.mode + '/sdf', ]


        # return [self.mode + '/sdf', self.mode + '/train.csv'] if self.mode == 'dev' or self.mode== 'valid' else [self.mode + '/sdf', ]
    ...
    def alchemy_nodes(self, g):
        ...
        for n, d in g.nodes(data=True):
            ...
            # Atomic number
#             h_t.append(d['a_num'])
            h_t += [int(d['a_num']-1 == x) for x in list(range(MAX_ATOMIC_NUM))]
#             print(d['a_num'])
            # Acceptor
...
#             h_t.append(d['num_h'])
            h_t += [int(d['num_h']==x) for x in [0,1,2,3,4]]
            feat.append((n, h_t))
    ...
    def alchemy_edges(self, g):
        ...
        for n1, n2, d in g.edges(data=True):
            ...
            tmpbin=0
            if float(d['distance'])*2<4:
                tmpbin=0
            elif float(d['distance'])*2>=12:
                tmpbin=9
            else:
                tmpbin=int(float(d['distance'])*2)-3
            e_t += [int(tmpbin==x) for x in list(range(10))]
            e_t.append(float(d['distance']))
            e[(n1, n2)] = e_t
    ...
    def sdf_graph_reader(self, sdf_file):
        ...
        l = torch.FloatTensor(self.target.loc[int(sdf_file.stem)].tolist()).unsqueeze(0) \
                if self.mode == 'dev' or self.mode == 'valid' else torch.LongTensor([int(sdf_file.stem)])
        ...
        for i in range(mol.GetNumAtoms()):
            for j in range(mol.GetNumAtoms()):
                ...
                tmpdis=np.linalg.norm(geom[i] - geom[j])
                if e_ij is not None:
                    g.add_edge(i, j, b_type=e_ij.GetBondType(),distance=tmpdis)
                else:
                    g.add_edge(i, j, b_type="NoBond",distance=tmpdis)
    ...
    def process(self):
        if self.mode == 'dev' or self.mode =='valid':
