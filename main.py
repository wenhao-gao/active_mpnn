...
parser.add_argument('--datasetPath', default='./train/sdf/train/', help='dataset path')
parser.add_argument('--logPath', default='./log/mpnn/', help='log path')
parser.add_argument('--plotLr', default=False, help='allow plotting the data')
parser.add_argument('--plotPath', default='./plot/mpnn/', help='plot path')
parser.add_argument('--resume', default='./checkpoint/mpnn/',
...
def main():
    ...
    data_train = datasets.Alchemy(root=root, csv_file='./train/train_train.csv')
    data_valid = datasets.Alchemy(root=root, csv_file='./train/train_valid.csv')
    ...
    print('\tStatistics')
    stat_dict = functions.get_graph_stats(data_valid, ['target_mean', 'target_std'])

    data_train.set_target_transform(lambda x: functions.normalize_data(x, stat_dict['target_mean'],
                                                                            stat_dict['target_std']))
    data_valid.set_target_transform(lambda x: functions.normalize_data(x, stat_dict['target_mean'],
                                                                            stat_dict['target_std']))

    # Data Loader
    train_loader = torch.utils.data.DataLoader(data_train,
                                               batch_size=args.batch_size, shuffle=True,
                                               collate_fn=functions.collate_g,
                                               num_workers=args.prefetch, pin_memory=True)
    valid_loader = torch.utils.data.DataLoader(data_valid,
                                               batch_size=args.batch_size, collate_fn=functions.collate_g,
                                               num_workers=args.prefetch, pin_memory=True)
    ...
    l_target = len(l)
    job_type = 'regression'
    model = MPNN(in_n, hidden_state_size, message_size, n_layers, l_target, type=job_type)
    del in_n, hidden_state_size, message_size, n_layers, l_target, job_type
    ...
    for epoch in range(0, args.epochs):
        ...
        functions.save_checkpoint({'epoch': epoch + 1, 'state_dict': model.state_dict(), 'best_er1': best_er1,
